...
```
참고로 관계 해석이란 어떤 데이터를 찾는지만 명시하는 선언적 언어로 관계 대수와 함께 DBMS 표준 언어인 SQL 의 이론적
기반을 제공한다.

관계 데이터베이스 사용시 어떤 데이터를 어떻게 찾는지 알고 있는 것이 중요하기 때문에 관게 대수에 대한 내용만 설명한다.
```
...
순수 관계 연산(데이터 모델을 위한 연산): 셀렉션, 프로젝션, 조인, 디비전, 개명
일반 집합 연산(수학의 집합 이론): 합집합, 차집합, 교집합, 카티전 프로덕트 
```

### 셀렉션과 프로젝션

셀렉션과 프로젝션은 관계 대수에서 가장 기본적인 연산 하나의 릴레이션을 대상으로 하는 단항 연산자이다.
```
* 셀렉션 σ

셀렉션을 사용하면 릴레이션에서 투플을 추출할 수 있다.

ex) 마당 서점에서 판매 하는 도서 중 8000 원 이하인 도서를 검색하시오.
σ가격<=8000 (도서)

ex) 가격이 8000 원 이하이고 도서 번호가 3 이상인 도서
σ가격<=8000^도서번호>=3 (도서)

조건이 여러개인 경우 ^(and), V(or), ¬(not) 기호를 사용해서 표현한다. (도서) 는 릴레이션을 의미한다.
```
```
* 프로젝션 π

프로젝션을 사용하면 릴레이션의 속성을 추출할 수 있다.

ex) 신간 도서 안내를 위해 (이름,주소,핸드폰) 의 카탈로그 주소록을 만드시오.
π이름, 주소, 핸드폰 (고객) 

릴레이션에서 해당 속성의 값을 추출한다.
```
### 집합 연산 (합집합, 교집합, 차집합, 카티전 프로덕트)

카티전 프로덕트를 제외한 합집합, 교집합, 차집합 은 두 릴레이션의 차수 및 도메인과 속성의 순서가 동일해야 한다. 이 조건을 합병 가능이라고 한다.

이때 속성의 이름은 동일하지 않아도 되며, 반환되는 릴레이션의 속성 이름은 첫 번째 릴레이션의 이름과 같다. 

```
* 합집합

ex) 마당서점 A 와 B 지점의 도서 릴레이션을 서로 합하시오
도서 A U 도서 B
```
```
* 교집합

ex) 마당 서접의 두 지점에서 동일하게 보유하고 있는 도서 목록을 보이시오.
도서 A U 도서 B
```
```
* 차집합

ex) 마당 서점의 두 지점 중 지점 A 에서만 보유하고 있는 도서 목록을 보이시오.
도서A - 도서 B
```
```
* 카티전 프로덕트

수학에서 말하는 카티전 프로덕트와 마찬가지로 각 릴레이션 원소 집합(투플 집합) 을 하나로 합칠 때 사용하는 카티전 프로덕트를
DBMS 에서도 사용할 수 있다. 

첫 번째 릴레이션의 차수가 4, 카디날리티가 3 이고 두 번째 릴레이션의 차수가 5, 카디날리티가 4 라면 카티전 프로덕트로 계산한
결과 릴레이션의 차수는 9(4+5), 카디날리티는 12(3*4) 가 된다.

고객 X 주문 = (고객 주문) 릴레이션 으로 결과 릴레이션의 오른쪽에 두 번째 릴레이션을 연결시키며 두 릴레이션이 동일한 속성
이름을 가지고 있으면 충돌할 수도 있다. 이때는 속성 이름을 릴레이션이름.속성 이름으로 표현하거나 순서를 사용하여 표현한다.

하지만 단순히 카티전 프로덕트만 사용해서는 유용한 자료로 활용하기 힘들다. 단순히 집합을 합치는 것이기 때문에 데이터가 의미
없이 합쳐진다.
```
### 조인 (세타조인, 동등조인, 자연조인, 외부조인, 세미조인)

조인은 두 릴레이션을 카티전 프로덕트 연산을 한 후 셀렉션 연산을 수행한 것이다. 셀렉션 연산의 조건은 속성 값이 같은지 비교하는 식이다. 

조인을 수행하기 위해서는 두 릴레이션의 '조인에 참여하는 속성' 이 서로 동일한 도메인으로 구성되어야 한다.
...
세타조인은 조인에 참여하는 두 릴레이션의 속성 값을 비교해서 조건을 만족하는 투플만 반환하다. {=, ≠, ≤, ≥ <, >}
```
```
* 자연 조인 ⋈N

ex) 고객과 고객의 주문 사항을 모두 보여주되 같은 속성은 한번만 표시하시오.
고객 ⋈N 고객.고객번호 = 주문.고객번호 주문

자연 조인을 사용하면 카티전 프로덕트시 두번 째 릴레이션에서 첫번 째 릴레이션과 중복되는 속성이 나타나지 않는다.
```
```
* 외부 조인 ⟕,⟖,⟗ 

외부 조인에는 왼쪽, 오른쪽, 완전 외부 조인이 있다. 각 릴레이션의 위치를 기준으로 레프트 외부 조인을 하면 왼쪽에 있는 릴레이션의
속성 값이 모두 포함되고 오른쪽 릴레이션에서 조인되는 조건에 맞는 값이 없으면 오른쪽 릴레이션의 속성 값을 NULL 로 반환한다.

오른쪽의 경우도 마찬가지로 값이 없으면 NULL 값을 출력한다. 참고로 외부 조인은 자연 조인의 확장된 형태이므로 위치 기준 중복 속성을
출력하지 않는다.

완전 외부 조인을 하면 각 릴레이션의 값을 조인하면서 조인 조건이 없는 값들은 NULL 로 채워진다.
```
```
* 세미 조인 ⋉, ⋊	

세미조인은 각 릴레이션을 자연 조인 한 후 기호에서 닫힌 쪽 릴레이션의 투플만 반환한다. 즉 자연 조인된 결과의 닫힌 쪽 릴레이션의
조건에 맞는 값을 출력한다.
```
### 디비전 ÷
```
디비전 연산은 특정 값들을 모두 갖고 있는 투플을 찾을 때 사용한다. 실제로 자주 사용되지 않으며 조인 연산의 조합으로 디비전 연산을 수행할
수 있다.
