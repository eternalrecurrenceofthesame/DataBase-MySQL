...
분해할 수 있다. 복합 속성은 속성의 타원 아래 단순 속성 타원을 추가해서 연결한다. 
```
```
* 단일 값 속성과 다중 값 속성
...
세 개의 개체가 관게를 맺는다. 자동차 회사 직원은 부품을 조립하여 하나의 자동차를 만드는 프로젝트를 수행하는 관계를 맺고 있다.
```
### 관계 대응수에 대하여

관계 대응 수에 따른 관계 타입의 유형으로 일대일, 일대다, 다대일, 다대다 관계가 있다. 
```
...
관계 대응수를 IE 표기 법으로 나타낼 때도 개체의 반대편에 표시하며 반대편을 보고 관계를 읽으면 된다.
...
```
예를 들면 학생은 강좌를 수강할 때 휴학 등의 이유로 강좌에 개체에 모두 참여할 수 없으므로 한줄로 표시하며 강좌는 폐광 되는 과목 없이
수강 신청을 하는 학생이 반드시 있다고 가정하므로 전체 참여인 두 줄로 표시한다. 333 p
```
### 역할

일반적으로 역할은 관계 다이어 그램으로 관계를 알 수 있다면 생략하지만 관계에서 약할이 명확하지 않으면 반드시 표기한다. 333 p 그림 참고 
...
### IE 표기법
```
강한 개체 타입은 서로 비식별자 관계인 점선 관계로 표현한다. (부모 개체의 키가 일반 속성에 포함되는 관계)
약한 개체 타입은 실선 관게로 표현한다.
